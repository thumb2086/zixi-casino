      ...
      <div className="min-h-screen bg-surface text-white pb-32">
        ...
        <main className="app-shell pt-24 flex flex-col gap-6 max-w-md mx-auto">
          <div className="flex gap-2">
            ...
            <button onClick={() => setCompanyType("semiconductor")} className={`flex-1 py-3 rounded-xl font-black ${companyType === "semiconductor" ? "bg-accent text-black" : "bg-card text-white"}`}>{t("company_type_semiconductor")}</button>
          </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-2xl bg-card p-4 border border-border/10">
                    <p className="text-caption text-secondary font-bold">{t("company_type_ai")}</p>
      ...
      {data?.companies?.map((c: any) => (
        <div key={c.id} className="rounded-2xl bg-card p-4 border border-border/10 space-y-3">
          <div>
            ...
            <p className="text-caption text-secondary">{t("company_info", { type: t(c.companyType === "ai" ? "company_type_ai" : "company_type_semiconductor"), level: c.level, cash: c.data?.cash?.toLocaleString() || 0 })}</p>
          </div>
          <div className="flex items-center gap-2">
            <input
