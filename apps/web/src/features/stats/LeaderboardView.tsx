  ...
  return (
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {kings.length === 0 && (
            <span className="text-caption text-secondary">?´ÁÑ°Ë≥áÊ?</span>
          )}
        </div>
      </div>
...
export default function LeaderboardView() {
  ...
  const categoryTabs = [
    { id: 'xp' as LeaderboardCategory, icon: Trophy, label: 'Á∂ìÈ?Ê¶? },
    { id: 'asset' as LeaderboardCategory, icon: Wallet, label: 'Ë≥áÁî¢Ê¶? },
  ];

  const metricLabels: Record<LeaderboardCategory, string> = { xp: 'Á∂ìÈ?', asset: 'Ë≥áÁî¢' };
