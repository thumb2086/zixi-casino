...
export default function ShopView() {
  ...
  async function handleBuyChest(chestType: string, quantity: number = 1) {
    if (!sessionId) return;
    const label = chestType === 'common' ? '?ÆÈÄ? : chestType === 'rare' ? 'Á®Ä?? : chestType === 'epic' ? '?≤Ë©©' : '?≥Â?';
  ...
  async function handleConvertYjc() {
    ...
    if (!amount || amount < CONVERSION_RATE) {
      setMsg(`???Ä‰ΩéÂ???${CONVERSION_RATE.toLocaleString()} ZXC`);
      setTimeout(() => setMsg(null), 3000);
      return;
    }
    setConverting(true);
    ...
    try {
      ...
      if (res.data?.success) {
        setMsg(`???êÂ??åÊ? ${res.data.data?.yjcAmount || (amount / CONVERSION_RATE)} YJC`);
        setConvertZxc('');
        fetchItems();
      } else {
  ...
  async function handleConvertZxcFromYjc() {
    ...
    try {
      ...
      if (res.data?.success) {
        setMsg(`???êÂ??åÊ? ${res.data.data?.zxcAmount || (yjcNum * CONVERSION_RATE).toLocaleString()} ZXC`);
        setConvertYjc('');
        fetchItems();
      } else {
        setMsg(`??${res.data?.error?.message || res.data?.error || '?åÊ?Â§±Ê?'}`);
      ...
      <header className="fixed top-0 w-full z-50 bg-[#0e0e0e]/90 backdrop-blur-xl border-b border-[#494847]/15">
        <div className="flex items-center justify-between px-6 py-4 max-w-2xl mx-auto">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-extrabold tracking-tight text-xl text-[#fcc025] uppercase italic">?ÜÂ?</h1>
          </div>
        </div>
      ...
      <main className="pt-28 px-6 max-w-2xl mx-auto space-y-6">
        <section className="bg-[#1a1919] rounded-2xl p-4 border border-[#494847]/20 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-black uppercase tracking-widest text-[#adaaaa]">ZXC È§òÈ?</span>
          </div>
        ...
        <section className="bg-[#1a1919] rounded-2xl p-4 border border-[#494847]/20">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-black uppercase tracking-widest text-[#adaaaa]">‰ΩëÊà©Âπ?YJC</span>
          ...
          <div className="flex items-center gap-2">
            <input type="number" min={CONVERSION_RATE} step={CONVERSION_RATE} placeholder={`?ÄÂ∞?${CONVERSION_RATE.toLocaleString()}`} value={convertZxc} onChange={e => setConvertZxc(e.target.value)} className="flex-1 bg-[#0e0e0e] text-white text-xs font-bold rounded-lg px-3 py-2 border border-[#494847]/30 outline-none focus:border-[#fcc025] placeholder:text-[#494847]" />
            <button onClick={handleConvertYjc} disabled={converting || !convertZxc || !sessionId} className="shrink-0 text-sm font-black uppercase tracking-widest bg-[#4fc3f7] text-[#0e0e0e] px-4 py-2 rounded-lg disabled:opacity-50">
              {converting ? <Loader2 size={12} className="animate-spin" /> : '?åÊ?'}
            </button>
          </div>
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#494847]/20">
            <input type="number" min="0.0001" step="0.0001" placeholder="YJC ?∏È?" value={convertYjc} onChange={e => setConvertYjc(e.target.value)} className="flex-1 bg-[#0e0e0e] text-white text-xs font-bold rounded-lg px-3 py-2 border border-[#494847]/30 outline-none focus:border-[#fcc025] placeholder:text-[#494847]" />
            <button onClick={handleConvertZxcFromYjc} disabled={converting || !convertYjc || !sessionId} className="shrink-0 text-sm font-black uppercase tracking-widest bg-[#fcc025] text-black px-4 py-2 rounded-lg disabled:opacity-50">
              {converting ? <Loader2 size={12} className="animate-spin" /> : '?çÂ??åÊ?'}
            </button>
          </div>
        </section>
        ...
        <section className="bg-[#1a1919] rounded-2xl p-6 border border-[#494847]/20">
          ...
          {!loading && visibleItems.length === 0 && (
            <p className="text-sm text-[#adaaaa] text-center py-8">?ÆÂ??´ÁÑ°?ÜÂ?</p>
          )}
        ...
        <section className="bg-[#1a1919] rounded-2xl p-6 border border-[#494847]/20">
          ...
          {pawnTab === 'items' && (
          <>
          <p className="text-sm text-[#adaaaa] mb-4">Â∞á‰??ÄË¶ÅÁ??ìÂÖ∑?∏Áï∂?õÂ? ZXC</p>
          {invItems.length === 0 ? (
            <p className="text-sm text-[#adaaaa] text-center py-8">?´ÁÑ°?ØÂÖ∏?∂Á??ìÂÖ∑</p>
          ) : (
            <div className="space-y-3">
          ...
          {pawnTab === 'stocks' && (
          <>
          <p className="text-sm text-[#adaaaa] mb-4">‰ª•Â???70% ?∫ÂîÆ?°Á•®ÔºåÁ??≥Ë???ZXC</p>
          {stockHoldings.length === 0 ? (
            <p className="text-sm text-[#adaaaa] text-center py-8">?´ÁÑ°?ÅËÇ°</p>
          ) : (
            <div className="space-y-3">
                ...
                return (
                  <div key={stock.symbol} className="flex items-center gap-4 bg-[#0e0e0e] rounded-xl p-4 border border-[#494847]/20">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-[#adaaaa]">{stock.qty} ??¬∑ ?áÂÉπ {Number(stock.avgPrice || 0).toLocaleString()} ZXC</p>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-sm text-[#adaaaa]">Â∏ÇÂÄ?{Math.round(marketPrice * stock.qty).toLocaleString()} ZXC</span>
