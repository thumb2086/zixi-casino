        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
           ...
           <section className="bg-[#1a1919] rounded-2xl p-8 border border-[#494847]/10 space-y-6">
              <div className="flex items-center gap-2">
                 ...
                 <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-[#adaaaa]">SYSTEM EVENTS</h3>
              </div>
              <div className="space-y-4 max-h-[350px] overflow-y-auto pr-4 hide-scrollbar">
                 ...
                 {recentLogs.map((ev: any, i: number) => (
                    <div key={ev.id || i} className="bg-[#0e0e0e] rounded-xl p-4 border border-[#494847]/5 space-y-2 group hover:border-[#fcc025]/30 transition-all">
                       <div className="flex items-center justify-between">
                          <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-sm ${ev.severity === 'error' || ev.severity === 'fatal' ? 'bg-red-500/10 text-red-500' : ev.severity === 'warn' ? 'bg-amber-500/10 text-amber-400' : 'bg-[#fcc025]/10 text-[#fcc025]'}`}>[{ev.kind}]</span>
