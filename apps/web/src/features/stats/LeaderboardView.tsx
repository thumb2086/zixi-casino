  ...
  return (
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold uppercase tracking-widest text-secondary">王者榜單</span>
        </div>
        <div className="flex items-center gap-3">
          {remainingTime ? (
            <span className="text-sm font-bold text-accent/60">{remainingTime}</span>
          ) : totalCount !== undefined ? (
            <span className="text-sm font-bold text-secondary">共 {totalCount} </span>
          ) : null}
          {kings.length === 0 && (
            <span className="text-caption text-secondary">暫無資料</span>
          )}
        </div>
      </div>
