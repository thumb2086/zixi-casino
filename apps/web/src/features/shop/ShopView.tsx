...
export default function ShopView() {
  ...
  async function handleBuyChest(chestType: string, quantity: number = 1) {
    if (!sessionId) return;
    const label = ({ common:'普通', rare:'稀有', epic:'史詩', legendary:'傳說', mythic:'神話', chaos:'混沌', abyss:'深淵', oracle:'神諭' } as Record<string,string>)[chestType] || chestType;
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        const discountText = d.discount > 0 ? ` (折扣${(d.discount * 100).toFixed(0)}%)` : '';
        setMsg(`購買${quantity} x ${label}寶箱 已放入背包${discountText}`);
  ...
  const handleCancelListing = async (listingId: string) => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        setMsg(`錯誤：${res.data?.error || '請求失敗'}`);
      }
    } catch (err: any) {
      setMsg(`錯誤：${err?.response?.data?.data?.error || err?.message || '請求失敗'}`);
    }
  };
                        ...
                        {bundle.map((sub: any, i: number) => {
                            ...
                            return (
                              <div key={i} className="flex items-center gap-1.5 text-sm">
                                  ...
                                  {(sub.qty || 1) > 1 && <span className="text-secondary">x{sub.qty}</span>}
        ...
        <section className="bg-card rounded-2xl p-6 border border-border/20">
          ...
          {stockHoldings.length === 0 ? (
            ...
            <div className="bg-surface rounded-xl p-4 border border-border/20 mb-4">
              <div className="text-xs font-bold uppercase tracking-widest text-secondary mb-2">?倉總</div>
              <div className="grid grid-cols-2 gap-3 text-sm">
                {(() => {
                  ...
                  return (
                    <>
                      <div>
                        <span className="text-secondary">市</span>
                      ...
                      <div>
                        <span className="text-secondary">?本</span>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          ...
                          <span className={`text-xs font-bold ${isUp ? 'text-emerald-400' : 'text-red-400'}`}>
                            {isUp ? '漲' : '跌'} {Math.abs(dayChange).toFixed(2)}%
                          </span>
                        ...
                        <div className="flex items-center gap-3 mt-0.5">
                          ...
                          <span className="text-xs text-secondary">?價 {formatNumber(Number(stock.avgPrice || 0), numberMode)}</span>
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-1 shrink-0">
        ...
        <section className="bg-card rounded-2xl p-6 border border-border/20">
          ...
          <div className="flex gap-2 mb-4">
            ...
            <button onClick={() => { setMarketTab('mine'); fetchMyListings(); }}
              className={`text-sm font-black uppercase tracking-widest px-3 py-1 rounded-lg ${marketTab === 'mine' ? 'bg-accent text-black' : 'bg-[#494847]/30 text-secondary'}`}>我的出售</button>
          </div>
          ...
          {showSellForm && (
            <div className="bg-surface rounded-xl p-4 border border-border/20 mb-4 space-y-3">
              ...
              <button onClick={handleCreateListing} disabled={!sellItemId || !sellPrice}
                className="w-full bg-accent text-black font-black text-sm py-2 rounded-lg disabled:opacity-50">?賣</button>
