...
| **v1.0.19** | 股市調整 + 交易效能 | 背景 tick、admin 端點、清算修正、前端極速反饋 | ⏳ |
| **v1.0.20** | 上鏈系統 | 上鏈排隊佇列、上鏈公開頁面 | ⏳ |
| **v1.0.21** | 交易紀錄 + 動態重構 | 交易紀錄系統完善、公開動態保留市場與錢包、移除重複錢包動態 | ⏳ |
| **v1.0.22** | 全站快取 + 監控 | 管理頁面載入優化、伺服器運行時間、重新登入時間反饋圖表 | ⏳ |
| **v1.0.23** | 公司系統 | `/upgrade-fab`、`/deposit`、招聘押金、export、測試 | ⏳ |
| **v1.0.24** | 排版檢查 | 全站頁面 UI 排列、文字溢出、RWD 斷點、元件間距一致性檢查與修正 | ⏳ |
