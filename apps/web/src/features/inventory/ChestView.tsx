...
export default function ChestView() {
  ...
  const handleOpen = async (chestId: string, quantity: number) => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        showToast(res.data?.error || '?ãÂ?Â§±Ê?');
      }
    } catch (err: any) {
      showToast(err?.response?.data?.error || 'Á∂≤Ë∑Ø?ØË™§');
    } finally {
      setOpening(false);
    }
  };
  ...
  const useAllTokens = async () => {
    ...
    setUseStatusMessage('Ê≠?ú®?åÊ??®ÈÉ®‰ª?π£...');
    try {
      ...
      if (res.data?.success) {
        ...
        showToast(`?êÂ?‰ΩøÁî® ${d.itemCount} ?ãÁâ©?ÅÔ??≤Â? ${parts.join(' + ')}`);
        await refreshInventory();
        queryClient.invalidateQueries({ queryKey: ['wallet-summary'] });
      } else {
        showToast(res.data?.error || '?åÊ?Â§±Ê?');
      }
    } catch (err: any) {
      showToast(err?.response?.data?.error || '?åÊ?Â§±Ê?');
    } finally {
  ...
  const useItem = async (itemId: string, quantity: number = 1) => {
    setUseStatusMessage('Ê≠?ú®?ïÁ?...');
    try {
      const res = await api.post('/api/v1/inventory/use', { itemId, quantity });
      if (res.data?.success) {
        const d = res.data.data;
        if (d.currencyGranted > 0) {
          showToast(`?≤Â? ${nf(d.currencyGranted)} ${d.currencyToken === 'yjc' ? 'YJC' : 'ZXC'}`);
        } else if (d.effectSummary) {
          showToast(d.effectSummary);
        } else {
          showToast(`?êÂ?‰ΩøÁî® ${quantity} ?ãÁâ©?Å`);
        }
        await refreshInventory();
        queryClient.invalidateQueries({ queryKey: ['wallet-summary'] });
      } else {
        showToast(res.data?.error || '‰ΩøÁî®Â§±Ê?');
      }
    } catch (err: any) {
      showToast(err?.response?.data?.error || '‰ΩøÁî®Â§±Ê?');
    } finally {
      setUseStatusMessage(null);
    }
  };
      ...
      <header className="fixed top-0 z-50 w-full border-b border-border/20 bg-surface/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-2xl items-center justify-between px-6 py-4">
          ...
          <div className="text-right">
            <p className="text-[10px] font-bold uppercase tracking-widest text-secondary">Á©∫È?</p>
      ...
      <main className="px-6 pt-24 space-y-8">
        ...
        <div className="flex gap-2 border-b border-border/20 pb-3">
          {(['chests', 'tokens', 'buffs', 'collection'] as const).map((tab) => (
            ...
            >
              {{ chests: 'ÂØ∂ÁÆ±', tokens: '‰ª?π£', buffs: '?†Ê?', collection: '?∂Ë?' }[tab]}
            </button>
          ))}
        </div>
        ...
        <section className="space-y-4">
          <h2 className="text-sm font-black uppercase tracking-[0.2em] text-secondary mb-4">?ØÈ??üÂØ∂ÁÆ?/h2>
        ...
        {invTab === 'tokens' && (
          <section className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-black uppercase tracking-[0.2em] text-secondary">‰ª?π£</h2>
              {(() => {
                ...
                return tokenItems.length > 0 ? (
                  ...
                  >
                    {usingAllTokens ? '?ïÁ?‰∏?..' : `?®ÈÉ®‰ΩøÁî® (??{totalTokens.toLocaleString()} ZXC)`}
                  </button>
                ) : null;
              })()}
            </div>
            {(!groupedItems['token'] || groupedItems['token'].length === 0) ? (
              <p className="text-sm text-secondary text-center py-8">?´ÁÑ°‰ª?π£?©Â?</p>
            ) : (
                  ...
                  <div key={item.id} className="flex flex-col rounded-xl border border-border/20 bg-card p-3">
                    ...
                    <div className="mt-auto flex flex-wrap gap-2">
                      ...
                      <button onClick={() => useItem(item.id, parseInt(useQty[item.id] || '1', 10) || 1)}
                        className="flex-1 bg-accent text-black font-black text-sm py-2 rounded-lg hover:bg-[#e6ad03]">‰ΩøÁî®</button>
                      <button onClick={() => setGiftDialog({ itemId: item.id, name: item.name, maxQty: item.quantity })}
                        className="flex-1 border border-accent text-accent font-black text-sm py-2 rounded-lg hover:bg-accent/10">Ë¥àÈÄ?/button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {invTab === 'buffs' && (
          <section className="space-y-4">
            <h2 className="text-sm font-black uppercase tracking-[0.2em] text-secondary">?†Ê??©Â?</h2>
            {(!groupedItems['buff'] || groupedItems['buff'].length === 0) ? (
              <p className="text-sm text-secondary text-center py-8">?´ÁÑ°?†Ê??©Â?</p>
            ) : (
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        {invTab === 'collection' && (
          <section className="space-y-6">
            <h2 className="text-sm font-black uppercase tracking-[0.2em] text-secondary">?∂Ë?</h2>
                        ...
                        {(type === 'avatar' || type === 'title') && (
                          <div className="mt-auto flex gap-2">
                            <button onClick={() => useItem(item.id)}
                              className="flex-1 border border-accent text-accent font-black text-sm py-2 rounded-lg hover:bg-accent hover:text-black">Ë£ùÂ?</button>
                            <button onClick={() => setGiftDialog({ itemId: item.id, name: item.name, maxQty: item.quantity })}
                              className="flex-1 border border-accent text-accent font-black text-sm py-2 rounded-lg hover:bg-accent/10">Ë¥àÈÄ?/button>
                          </div>
                        )}
            ...
            {['avatar', 'title', 'collectible', 'chest_key'].every(t => !groupedItems[t]?.length) && (
              <p className="text-sm text-secondary text-center py-8">?´ÁÑ°?∂Ë??©Â?</p>
            )}
          </section>
        )}
      </main>
                ...
                <div className="text-center mb-4">
                  <div className="inline-flex items-center gap-3 bg-gradient-to-br from-[#fcc025]/20 to-[#e6ad03]/10 border border-accent/40 rounded-2xl px-6 py-4 shadow-lg shadow-[#fcc025]/5">
                    ...
                    <div className="text-left">
                      <p className="text-xs font-bold uppercase tracking-widest text-secondary">?çË?Ë£úÂ?</p>
          ...
          >
            ...
            >
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-black">Ë¥àÈÄ?{giftDialog.name}</h2>
                <button onClick={() => setGiftDialog(null)}>
                  <X className="w-5 h-5 text-secondary" />
                </button>
              </div>

              <label className="block text-sm font-bold text-secondary mb-1">?•Êî∂??/label>
              <select
                value={giftAddress}
              ...
              <label className="block text-sm font-bold text-secondary mb-1">?∏È?</label>
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (res.data?.success) {
                        showToast('Ë¥àÈÄÅÊ??üÔ?');
                        setGiftDialog(null);
                        setGiftAddress('');
                        setGiftQty('1');
                        await refreshInventory();
                      } else {
                        showToast(res.data?.error || 'Ë¥àÈÄÅÂ§±??);
                      }
                    } catch (err: any) {
                      showToast(err?.response?.data?.data?.error || err?.response?.data?.error || 'Ë¥àÈÄÅÂ§±??);
                    } finally {
