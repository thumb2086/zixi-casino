...
function formatExpires(expiresAt?: string | null): string {
  ...
  if (diff <= 0) return 'Â∑≤È???;
  const mins = Math.ceil(diff / 60000);
  if (mins < 60) return `?©È? ${mins} ?ÜÈ?`;
...
export default function ChestView() {
  ...
  const useItem = async (itemId: string, quantity: number = 1) => {
    ...
    try {
      ...
      if (res.data?.success) {
        showToast(`?êÂ?‰ΩøÁî® ${res.data.data.usedCount} ?ãÁâ©?Å`);
      ...
      <header className="fixed top-0 z-50 w-full border-b border-[#494847]/15 bg-[#0e0e0e]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-2xl items-center justify-between px-6 py-4">
          ...
          <div className="text-right">
            <p className="text-[10px] font-black uppercase tracking-widest text-[#adaaaa]">Á©∫È?</p>
      ...
      <main className="mx-auto max-w-2xl px-6 pt-24 space-y-8">
            ...
            {inventory.activeBuffs.map((buff) => (
              <div key={buff.id} className="relative overflow-hidden rounded-xl border border-[#fcc025]/20 bg-[#1a1919] p-3">
                ...
                <div className="mt-2 text-[10px] font-bold text-[#adaaaa] opacity-60">
                  {buff.expiresAt ? formatExpires(buff.expiresAt) : '?üÊ?‰∏?}
                </div>
              </div>
            ))}
          </section>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-black uppercase tracking-[0.2em] text-[#adaaaa]">?ØÈ??üÂØ∂ÁÆ?/h2>
            <div className="flex items-center gap-2">
        ...
        <section className="space-y-6">
          <h2 className="text-sm font-black uppercase tracking-[0.2em] text-[#adaaaa]">?ëÁ??©Ë?</h2>
          {chests.length > 0 && inventory.items.length === 0 ? (
             <div className="rounded-2xl border-2 border-dashed border-[#494847]/20 py-12 text-center">
                ...
                <p className="text-sm font-bold text-[#adaaaa]">?ÆÂ?Ê≤íÊ?‰ªª‰??©Ë?</p>
             </div>
          ) : (
                ...
                <div className="text-center mb-4">
                  <div className="inline-flex items-center gap-3 bg-gradient-to-br from-[#fcc025]/20 to-[#e6ad03]/10 border border-[#fcc025]/40 rounded-2xl px-6 py-4 shadow-lg shadow-[#fcc025]/5">
                    ...
                    <div className="text-left">
                      <p className="text-xs font-black uppercase tracking-widest text-[#adaaaa]">?çË?Ë£úÂ?</p>
        ...
        {giftDialog && (
          ...
          >
            ...
            >
              ...
              <label className="block text-sm font-bold text-[#adaaaa] mb-1">?•Êî∂??/label>
              <select
                value={giftAddress}
              ...
              <label className="block text-sm font-bold text-[#adaaaa] mb-1">?∏È?</label>
