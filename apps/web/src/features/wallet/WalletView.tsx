...
export default function WalletView() {
  ...
  const nextAirdropLabel = useMemo(() => {
    ...
    return `?éÊó• ${tomorrow.toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit' })}`;
  }, [canClaimAirdrop, t]);
        ...
        {profileData?.level ? (
          <section className="card-accent bg-card p-6 border border-border/10">
            <div className="flex items-center justify-between mb-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-secondary">Á∂ìÈ?Á≠âÁ?</p>
              ...
              <div className="text-right">
                ...
                {profileData.xpNextLevel > 0 && (
                  <p className="text-[10px] text-secondary mt-0.5">‰∏ã‰?Á¥?{nf(profileData.xpNextLevel)} XP</p>
                )}
              </div>
            </div>
          ...
          <div className="card-accent bg-card p-6 border border-border/10">
            ...
            <div className="flex items-center gap-3 mb-3">
              <div className="flex flex-col items-center bg-surface rounded-xl px-4 py-2">
                ...
                <span className="text-[10px] font-bold text-secondary">???Â§©Êï∏</span>
              </div>
              <div className="flex-1">
                <p className="text-xs font-bold text-secondary">{t('vault.next_available')}{nextAirdropLabel}</p>
                <p className="text-xs font-bold text-accent mt-1">?éÂãµ?çÁ? ?{(1 + checkinStreak * 0.05).toFixed(2)}</p>
              </div>
            </div>
          ...
          <div className="card-info bg-card p-6 border border-border/10 md:col-span-2 lg:col-span-1">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xs font-bold uppercase tracking-[0.18em] text-white">‰∫§Ê?Á¥Ä??/h2>
              </div>
              <Link to="/app/transactions" className="text-[10px] font-bold text-accent hover:underline">?•Á??®ÈÉ®</Link>
            </div>
            <div className="space-y-2">
              {summary.isLoading && <div className="text-xs text-secondary">ËºâÂÖ•‰∏?..</div>}
              {!summary.isLoading && (!walletSummary?.recentTransactions || walletSummary.recentTransactions.length === 0) && <div className="text-xs text-secondary">?´ÁÑ°‰∫§Ê?</div>}
                ...
                return (
                  <div key={tx.id} className="flex items-center justify-between rounded-xl border border-border/10 bg-surface px-4 py-3">
                    ...
                    <div className="text-right shrink-0 ml-3">
                      ...
                      <p className="text-[10px] text-secondary">{tx.status ? t('txStatus.' + tx.status, tx.status) : 'Â∑≤Á¢∫Ë™?}</p>
                    </div>
                  </div>
                );
              })}
