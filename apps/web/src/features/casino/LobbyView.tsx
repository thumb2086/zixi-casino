                  ...
                  <div key={m.id} className={`rounded-xl border ${locked ? 'border-border/10 opacity-50' : done ? 'border-accent/40' : 'border-border/20'} bg-surface p-4`}>
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <p className="text-[10px] text-secondary mt-0.5">{locked ? `?ÄË¶?VIP ${m.vip} ‰ª•‰?` : m.desc}</p>
                      </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      {locked ? null : done ? (
                        <button onClick={() => claimMission(m.id)} className="text-[10px] font-bold text-black bg-accent px-2 py-1 rounded-lg hover:brightness-110">?òÂ?</button>
                      ) : null}
                    </div>
                  </div>
                );
