...
export default function PerformanceView() {
  ...
  const metrics = [
    { icon: Clock, label: 'ä¼ºæ??¨é?è¡?, value: perfData?.uptimeLabel || '...', color: 'text-emerald-400' },
    { icon: Users, label: 'è¨»å??¨æˆ¶', value: nf(perfData?.users || 0), color: 'text-info' },
    { icon: Activity, label: 'æ´»è????', value: nf(perfData?.activeSessions || 0), color: 'text-accent' },
    { icon: Database, label: '24h äº¤æ???, value: nf(perfData?.tx24h || 0), color: 'text-warning' },
    { icon: Server, label: 'UPTIME', value: stats?.uptime ?? '...', color: 'text-emerald-400' },
    { icon: Zap, label: 'å¤±æ???, value: stats?.failureRate ?? '...', color: stats?.failureRate && parseFloat(stats.failureRate) > 5 ? 'text-danger' : 'text-accent' },
  ];
      ...
      <header className="fixed top-0 w-full z-50 bg-surface/90 backdrop-blur-xl border-b border-border/20">
        <div className="app-shell flex items-center justify-between py-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-extrabold tracking-tight text-xl text-accent uppercase italic">?ˆèƒ½??Ž§</h1>
          </div>
        </div>
      </header>
      ...
      <main className="app-shell pt-24">
          ...
          {metrics.map((m) => (
            <div key={m.label} className="bg-card p-5 rounded-2xl border border-border/10 flex flex-col gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-bold text-muted uppercase tracking-[0.2em]">{m.label}</span>
              </div>
        ...
        {stats?.nodes && (
          <section className="bg-card rounded-2xl p-6 border border-border/10">
            <h2 className="text-xs font-bold uppercase tracking-widest text-secondary mb-4">ç¯€é»žç???/h2>
        ...
        {stats?.logs && stats.logs.length > 0 && (
          <section className="bg-card rounded-2xl p-6 border border-border/10 mt-6">
            <h2 className="text-xs font-bold uppercase tracking-widest text-secondary mb-4">è¿‘æ?äº‹ä»¶</h2>
