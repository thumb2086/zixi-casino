    ...
    <div className="min-h-screen bg-[#0e0e0e] text-white font-manrope-emoji pb-32">
      <header className="fixed top-0 w-full z-50 bg-[#0e0e0e]/90 backdrop-blur-xl border-b border-[#494847]/15">
        <div className="flex items-center gap-3 px-6 py-4 max-w-2xl mx-auto">
          ...
          <h1 className="font-extrabold tracking-tight text-xl text-[#fcc025] uppercase italic">?∂Ë?Ê´?/h1>
        </div>
      </header>
      ...
      <main className="pt-20 px-6 max-w-2xl mx-auto">
        {loading ? (
          ...
          <div className="rounded-xl border border-[#494847]/20 bg-[#1a1919] p-12 text-center mt-8">
            ...
            <p className="text-sm text-[#adaaaa]">Â∞öÊú™?∂Ë?‰ªª‰??©Â?</p>
            <p className="text-xs text-[#494847] mt-1">?ãÂ?ÂØ∂ÁÆ±?âÊ??ÉÁç≤ÂæóÁ??âÊî∂?èÂ?</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-6">
            {items.map((item: any) => (
              ...
              >
                <div className="text-5xl mb-3">{item.icon || '?ì¶'}</div>
