...
function buildNoGapOptions(gate) {
    const lowerOuts = Math.max(0, gate.left.value - 1);
    const higherOuts = Math.max(0, 13 - gate.right.value);
    const lowerMultiplier = lowerOuts > 0 ? Math.max(1.2, Math.round(((15 - lowerOuts) / lowerOuts) * 10) / 10) : 0;
    const higherMultiplier = higherOuts > 0 ? Math.max(1.2, Math.round(((15 - higherOuts) / higherOuts) * 10) / 10) : 0;
    return {
...
function getShotMultiplier(gate, sideGuess) {
    const gap = gate.right.value - gate.left.value;
    if (gap > 1) return getMultiplier(gap);
    const options = buildNoGapOptions(gate);
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (playMode === "classic" && playAction === "gate") {
            ...
            const sideOptions = gap <= 1 ? buildNoGapOptions(gate) : null;
        ...
        if (playMode === "classic" && playAction === "shoot") {
            ...
            if (round.requiresSideGuess) {
                ...
                const options = round.sideOptions || buildNoGapOptions(gate);
                if ((sideGuess === "lower" && !options.lower.enabled) || (sideGuess === "higher" && !options.higher.enabled)) {
            ...
            const multiplier = Number(round.requiresSideGuess ? getShotMultiplier(gate, sideGuess) : (round.multiplier || getMultiplier(round.gap)));
        ...
        const multiplier = requiresSideGuess ? getShotMultiplier(gate, sideGuess) : getMultiplier(gap);
