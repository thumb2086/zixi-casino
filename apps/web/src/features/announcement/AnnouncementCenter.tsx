...
const REWARD_NAMES: Record<string, string> = {
  title_newbie: '?ùÂ???, title_gambler: 'Ë≥≠Â?', title_highroller: 'Ë±™ÂÆ¢',
  title_god: 'Ë≥≠Á?', title_member_1: '?ÆÈÄöÊ???, title_member_2: '?ÖÁ??ÉÂì°',
  title_member_3: '?Ä?åÊ???, title_member_4: '?ëÁ??ÉÂì°', title_member_5: '?ΩÈ??ÉÂì°',
  title_member_6: '?ΩÁü≥?ÉÂì°', title_member_7: '?üËæ∞?ÉÂì°', title_member_8: '?ìÊ??ÉÂì°',
  title_member_9: 'È©ïÈôΩ?ÉÂì°', title_member_10: '?≥Â??ÉÂì°', title_member_11: 'Â§©ÈÅ∏‰πã‰∫∫',
  title_member_12: '‰∏çÊúΩ?≥Â?', title_member_13: 'Ê∞∏Ê?Á•ûË©±', title_member_14: 'Ê∑±Ê∑µ?ùË?',
  title_member_15: '?õÁ©∫Ë°åËÄ?, title_member_16: 'Ê∑∑Ê?‰∏ªÂÆ∞', title_member_17: '?ΩÈ?Á∑®Á???,
  title_member_18: '?ÇÂ?ÂÆàË≠∑??, title_member_19: '?µ‰?‰πãÂ?', title_member_20: 'ÁµÇÁ?ÂØ©Âà§',
  title_member_21: 'Ëº™Ëø¥‰πã‰∏ª', title_member_22: 'Â§™Â?‰πãÂ?', title_member_23: '?°Ê•µ‰πãÂ?',
  title_member_24: 'Â§ßÈ?Ê≠∏‰?', title_member_25: 'È¥ªË??ñ‰∫∫', title_member_26: 'Â§©È??ñË∫´',
  title_member_27: 'Ë¶èÂ??∂Â???, title_member_28: 'ÂÆáÂ?‰πãÂ?', title_member_29: '?¨Áâ©‰πãÊ?',
  title_member_30: 'Ë∂ÖË???, title_member_31: '?≥È?Á•ûÂ∫ß', title_member_32: 'Á•ûË´≠?Å‰???,
  chest_key_common: '?ÆÈÄöÂØ∂ÁÆ±Èë∞??, chest_key_rare: 'Á®Ä?âÂØ∂ÁÆ±Èë∞??,
  chest_key_epic: '?≤Ë©©ÂØ∂ÁÆ±?∞Â?', chest_key_legendary: '?≥Â?ÂØ∂ÁÆ±?∞Â?',
  chest_key_mythic: 'Á•ûË©±ÂØ∂ÁÆ±?∞Â?',
};
...
function formatRewardSummary(r: any, t: (k: string, d?: string) => string): string {
  if (!r || typeof r !== 'object') return t('rewardName.reward', '?éÂãµ');
  const parts: string[] = [];
  ...
  if (Array.isArray(r.items) && r.items.length > 0) {
    const labels = r.items.map((it: any) => {
      const name = it.name || resolveRewardName(it.id || '') || it.id || t('rewardName.item', '?ìÂÖ∑');
      const qty = it.qty || 1;
      return qty > 1 ? `${name} x${qty}` : name;
    });
    parts.push(labels.join(', '));
  }
  if (Array.isArray(r.avatars) && r.avatars.length) parts.push(`${t('rewardName.avatar', '?≠Â?')}: ${r.avatars.map((a: string) => resolveRewardName(a)).join(', ')}`);
  if (Array.isArray(r.titles) && r.titles.length) parts.push(`${t('rewardName.title', 'Á®±Ë?')}: ${r.titles.map((tl: string) => resolveRewardName(tl)).join(', ')}`);
  return parts.length ? parts.join(' + ') : t('rewardName.reward', '?éÂãµ');
}
...
export default function AnnouncementCenter() {
  ...
  async function claim(campaignId: string) {
    if (!sessionId) { setClaimMsg('Ë´ãÂ??ªÂÖ•'); return; }
    setClaimMsg(null);
    try {
      const res = await api.post(`/api/v1/rewards/campaigns/${encodeURIComponent(campaignId)}/claim`, { sessionId });
      const payload = res?.data?.data;
      if (payload?.error) { setClaimMsg(payload.error.message || payload.error.code || '?òÂ?Â§±Ê?'); return; }
      ...
      setClaimMsg(err?.response?.data?.data?.error?.message || err?.response?.data?.error?.message || err?.message || '?òÂ?Â§±Ê?');
    }
  }
      ...
      <header className="fixed top-0 w-full z-50 bg-surface/90 backdrop-blur-xl border-b border-border/20">
        <div className="flex items-center justify-between px-6 py-4 ">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-extrabold tracking-tight text-xl text-accent uppercase italic">?¨Â??áÊ¥ª??/h1>
          </div>
        </div>
      </header>
      ...
      <main className="pt-24 px-6 space-y-8">
        ...
        <div className="flex bg-card p-1.5 rounded-xl border border-border/20">
          {(['ANNOUNCEMENT', 'EVENTS', 'TRANSACTIONS'] as const).map((entry) => (
            ...
            >
              {entry === 'ANNOUNCEMENT' ? '?¨Â?' : entry === 'EVENTS' ? 'Ê¥ªÂ?' : '?ïÊ?'}
            </button>
          ))}
        </div>
                ...
                <section key={c.campaignId} className="rounded-2xl border border-border/20 bg-card p-4">
                  ...
                  {(c.startAt || c.endAt) && (
                    <p className="mt-2 text-sm text-secondary">
                      {c.startAt ? new Date(c.startAt).toLocaleString() : '?≥Âàª'} ~ {c.endAt ? new Date(c.endAt).toLocaleString() : '?°Ê???}
                    </p>
                  )}
                  ...
                  >
                    {c.claimed ? 'Â∑≤È??? : !sessionId ? 'Ë´ãÂ??ªÂÖ•' : '?òÂ??éÂãµ'}
                  </button>
                </section>
              ))
            )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4" onClick={() => setSuccessBundle(null)}>
          <div className="w-full max-w-sm rounded-2xl border border-border/20 bg-card p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="text-base font-black text-accent">?? ?òÂ??êÂ?</h3>
            ...
            <div className="mt-4 space-y-2 text-sm text-white">
              ...
              {Array.isArray(successBundle.items) && successBundle.items.map((it: any, i: number) => (
                <p key={i}>?ìÂÖ∑Ôºö{it.name || resolveRewardName(it.id) || it.id} {it.qty > 1 ? `?${it.qty}` : ''}</p>
              ))}
              {Array.isArray(successBundle.avatars) && successBundle.avatars.map((a: any, i: number) => (
                <p key={i}>?≠Â?Ôºö{a.name || resolveRewardName(a.id || a) || a.id || a}</p>
              ))}
              {Array.isArray(successBundle.titles) && successBundle.titles.map((t: any, i: number) => (
                <p key={i}>Á®±Ë?Ôºö{t.name || resolveRewardName(t.id || t) || t.id || t}</p>
              ))}
            </div>
            <button type="button" onClick={() => setSuccessBundle(null)} className="mt-4 w-full rounded-lg bg-accent px-3 py-2 text-sm font-black text-black">Á¢∫Â?</button>
          </div>
        </div>
      )}
